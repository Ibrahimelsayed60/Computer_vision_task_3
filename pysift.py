...
def SIFT_matching_keypoint(image1,image2):
...
def featured_image(image):
    start_time = time.process_time()
    kp1, des1 = computeKeypointsAndDescriptors(image)
...
def generateBaseImage(img, sigma, blur_assumed):

    ##Double the input image in size and applies Gaussian blur.
    img = resize(img,(0,0),fx=2,fy=2,interpolation=INTER_LINEAR)
    diff_in_sigma = sqrt( max((sigma*sigma) - ((2*blur_assumed)**2) , 0.01 ))
    result = GaussianBlur(img, (0,0), sigmaX=diff_in_sigma,sigmaY=diff_in_sigma)

    return result

def computeNumberOfOctaves(img_shape):

    ##### compute the number of times we can repeatedly halve an image until it becomes too small.
    result = int(round(log(min(img_shape)) / log(2) - 1))
    return result
...
def generateGaussianImages(image, num_octaves, gaussian_kernels):
    ##### need to actually generate our image pyramids
    gaussian_images = []

    for index_active in range(num_octaves):
        gaussian_images_octave = []
        gaussian_images_octave.append(image)
        for kernel in gaussian_kernels[1:]:
            ### Applying the Gaussian Filter in the image in X-direction and y-direction
            image = GaussianBlur(image, (0,0), sigmaX=kernel,sigmaY=kernel)
            gaussian_images_octave.append(image)
        gaussian_images.append(gaussian_images_octave)
